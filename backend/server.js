...
async function handleLienChiApi(request, response, url) { const id = url.pathname.match(/^\/api\/lien-chi\/([a-zA-Z0-9-]+)$/)?.[1]; if (request.method === 'GET' && url.pathname === '/api/lien-chi') return sendJson(response, 200, await readLienChi()); if (!isAuthorized(request)) return sendError(response, 401, 'Cần mật khẩu quản trị để thay đổi danh mục.'); const items = await readLienChi(); if (request.method === 'POST' && url.pathname === '/api/lien-chi') { const item = normalizeLienChi(await readRequestBody(request), { id: crypto.randomUUID(), sortOrder: items.length ? Math.max(...items.map(value => value.sortOrder)) + 1 : 1 }); items.push(item); await writeJson(LIEN_CHI_FILE, items); return sendJson(response, 201, item); } const index = items.findIndex(item => item.id === id); if (index < 0) return sendError(response, 404, 'Không tìm thấy Liên chi.'); if (request.method === 'PATCH') { items[index] = normalizeLienChi(await readRequestBody(request), items[index], index); await writeJson(LIEN_CHI_FILE, items); return sendJson(response, 200, items[index]); } if (request.method === 'DELETE') { items.splice(index, 1); await writeJson(LIEN_CHI_FILE, items); return sendJson(response, 200, { success: true }); } return sendError(response, 405, 'Phương thức hoặc đường dẫn API không được hỗ trợ.'); }
async function handleApi(request, response, url) { if (url.pathname.startsWith('/api/lien-chi')) return handleLienChiApi(request, response, url); if (request.method === 'GET' && url.pathname === '/api/clubs') return sendJson(response, 200, await readClubs()); return sendError(response, 404, 'Không tìm thấy API được yêu cầu.'); }
async function serveStaticFile(request, response, url) { const requestedPath = APP_ROUTES.has(url.pathname) ? 'index.html' : decodeURIComponent(url.pathname).replace(/^[/\\]+/, ''); const safePath = path.normalize(requestedPath).replace(/^[/\\]+/, ''); if (!safePath || safePath.startsWith('..')) return sendError(response, 404, 'Không tìm thấy trang hoặc tệp được yêu cầu.'); const filePath = path.resolve(FRONTEND_DIRECTORY, safePath); if (!filePath.startsWith(`${FRONTEND_DIRECTORY}${path.sep}`)) return sendError(response, 403, 'Không được phép truy cập tệp này.'); try { const content = await fs.readFile(filePath); response.writeHead(200, { 'Content-Type': MIME_TYPES[path.extname(filePath).toLowerCase()] || 'application/octet-stream', 'Cache-Control': 'no-store' }); response.end(request.method === 'HEAD' ? undefined : content); } catch (error) { if (error.code === 'ENOENT') return sendError(response, 404, 'Chưa có bản build React. Hãy chạy npm run build trước khi dùng máy chủ.'); throw error; } }
const server = http.createServer(async (request, response) => { try { const url = new URL(request.url, `http://${request.headers.host || 'localhost'}`); if (url.pathname.startsWith('/api/')) await handleApi(request, response, url); else if (request.method === 'GET' || request.method === 'HEAD') await serveStaticFile(request, response, url); else sendError(response, 405, 'Chỉ hỗ trợ GET hoặc HEAD cho tệp giao diện.'); } catch (error) { console.error(error); sendError(response, 400, error.message || 'Máy chủ không thể xử lý yêu cầu.'); } });
server.listen(PORT, () => console.log(`UET Navigator is running at http://localhost:${PORT}`));
