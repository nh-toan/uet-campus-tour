...
function createInitialCameraPosition(
  ...
  return [
    ...
    Math.cos(yawRadians) * CAMERA_ORBIT_RADIUS,
  ];
}
...
function PanoramaStatus({ detail, role, title }: PanoramaStatusProps) {
  return (
    <div className="campus-map-status" role={role}>
      <div className="campus-map-status__card">
        <p className="campus-map-status__title">{title}</p>
        <p className="campus-map-status__detail">{detail}</p>
      </div>
    </div>
  );
}
...
function PanoramaCanvas({ scene }: PanoramaCanvasProps) {
  ...
  useEffect(() => {
    ...
    const pendingTexture = loader.load(
      ...
      (error) => {
        if (active) {
          setLoadError(describeTextureError(error, scene.id));
        }
      },
    );
